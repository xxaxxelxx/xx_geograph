...
#exit
			...
			test -r $WORKDIR/geo.$TSTAMP.kml && rm -f $WORKDIR/geo.$TSTAMP.kml
			echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" > $WORKDIR/geo.$TSTAMP.kml
			echo "<kml xmlns=\"http://www.opengis.net/kml/2.2\">" >> $WORKDIR/geo.$TSTAMP.kml
			echo "<Document>" >> $WORKDIR/geo.$TSTAMP.kml
			echo "<name>$TSTAMP</name>" >> $WORKDIR/geo.$TSTAMP.kml
#			echo "<Style id=\"style_$CUSTOMER\"><IconStyle><Icon><href>http://maps.google.com/mapfiles/kml/paddle/red-stars-lv.png</href></Icon></IconStyle></Style>" >> $WORKDIR/geo.$TSTAMP.kml
			echo "<Style id=\"style_$CUSTOMER\"><IconStyle><scale>0.4</scale><Icon><href>https://storage.googleapis.com/support-kms-prod/SNP_2752125_en_v0</href></Icon></IconStyle></Style>" >> $WORKDIR/geo.$TSTAMP.kml
			for FOLDER_FILE in /tmp/geo.$CUSTOMER.*.folder; do
			    test -r $FOLDER_FILE || continue
			    echo "</Folder>" >> $FOLDER_FILE
			    cat $FOLDER_FILE >> $WORKDIR/geo.$TSTAMP.kml
			    rm -f $FOLDER_FILE
			done
			echo "</Document></kml>" >> $WORKDIR/geo.$TSTAMP.kml
			zip $WORKDIR/geo.$TSTAMP.kmz $WORKDIR/geo.$TSTAMP.kml > /dev/null 2>&1 && rm -f $WORKDIR/geo.$TSTAMP.kml 
		    )
		fi
	    fi
